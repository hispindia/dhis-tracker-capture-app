...
.controller('TEIRegistrationController', 
     ...
     */
    // update for INTPART  for disable attribute patient_identifier
    $scope.isDisabled = function (attribute) {
        if (attribute.code === 'person_unique_id' ) {
            return true;
        }
        else {
            return attribute.generated || $scope.assignedFields[attribute.id] || $scope.editingDisabled;
    ...
    $scope.registerEntity = function(){
        ...
        //console.log( $scope.selectedTeiForDisplay );
        var familyUniqueId = '';
        var memberUniqueId = '';
        for ( var i = 0; i < $scope.selectedTeiForDisplay.attributes.length; i++ ) {

            if( $scope.selectedTeiForDisplay.attributes[i].code === 'family_unique_id' &&
                $scope.selectedTeiForDisplay.attributes[i].value != undefined ){

                familyUniqueId = $scope.selectedTeiForDisplay.attributes[i].value;
            }
        ...
        // custom change for custom-id generation for member program
        var selectedOrgUnitCode = $scope.selectedOrgUnit.code;
        var orgUnitCodeUpperCase = selectedOrgUnitCode.substr(0, 4).toUpperCase();
        //console.log( $scope.tei );

        var houseIdentifierValue = "";
        // 6 digit random number
        var suffix = Math.floor(Math.random() * 1000000) + 1;

        if ($scope.selectedTei.Ehuk7obP0SK != undefined) {
            memberUniqueId = $scope.selectedTei.Ehuk7obP0SK;
            //houseIdentifierValue = tempStr.substr(0, 4).toUpperCase();
        }
        //$scope.generatedCustomId = orgUnitCodeUpperCase + "-" + tempStr + "-" + suffix;
        $scope.generatedCustomId = familyUniqueId + "-" + memberUniqueId;
