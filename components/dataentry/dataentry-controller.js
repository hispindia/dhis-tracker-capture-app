...
trackerCapture
    ...
    function (
      ...
      $scope.CustomDataElements = {
        ...
        cleanlinessWard: [
          ...
          "Ward smells of disinfectant",
          "Ward has bins for biomedical waste and garbage bins, which are not full",
          "Calculated Value - Cleanliness of Wards",
        ],
        handWashing: [
          "Running water present (tap is functioning, and water is coming out) or container with a functioning tap containing water",
          "Paper napkins /clean cloths/hand dryer available to dry hands",
          "Soap (bar/liquid) is available (no detergent)",
          "Alcohol based hand rub is available for hand hygiene",
          "Calculated Value - Hand-washing"
        ],
        toilets: [
          "Presence of toilets for male and female within the premise (PwD friendly)",
          "The toilet has a door, and each toilet is lockable from the inside",
          "Each lock is functional",
          "Doors cannot be locked from the outside",
          "Each toilet has running water or container with water with a ladle for flushing the toilet",
          "Each toilet has a place (inside or outside next to toilet) to wash hands with running water - or a water container with a functional tap containing water, paper napkins/soap",
          "Each toilet has a functional light (either electric or solar)",
          "Each toilet has no visible waste",
          "Each toilet has no flies",
          "No foul-smelling odour in each toilet (and may have air refresher)",
          "Calculated Value - Toilets",
        ],
        showerFacilities: [
          "Presence of at least one shower with running water or water container with water and a ladle containing water (at least half full) or a big water container for taking a shower and small container for flushing the toilet",
          "The room is lockable from the inside, and the lock is functional, and the room cannot be locked from the outside",
          "The shower has an exhaust fan or ventilation through tiles",
          "Calculated Value - Shower Facilities for Patients"
        ],
        medicalWaste: [
          "Garbage bin does not contain used syringes or needles",
          "Garbage bin/Biomedical waste bins available and not full",
          "Safety Box/Sharp containers available and not full",
          "Calculated Value - Medical Waste Handling"
        ],
      };
      ...
      var dashBoardLayoutPromise = DashboardLayoutService.get().then(function (
        response
      ) {
        $scope.dashBoardLayout = response;
        if (
          $scope.dashBoardLayout.customLayout &&
          $scope.dashBoardLayout.customLayout[$scope.selectedProgramId] &&
          $scope.dashBoardLayout.customLayout[$scope.selectedProgramId]
            .programStageTimeLineLayout
        ) {
          DashboardLayoutService.setProgramStageLayout(
            $scope.dashBoardLayout.customLayout[$scope.selectedProgramId]
              .programStageTimeLineLayout
          );
        } else if (
          $scope.dashBoardLayout.defaultLayout &&
      ...
      $scope.userAuthority = AuthorityService.getUserAuthorities(
        SessionStorageService.get("USER_PROFILE")
      );
      ...
      if (userProfile) {
        userSearchOrgUnits =
          userProfile.teiSearchOrganisationUnits &&
          userProfile.teiSearchOrganisationUnits.length > 0
            ? userProfile.teiSearchOrganisationUnits
            : userProfile.organisationUnits || [];
      }
      var storedBy =
        userProfile &&
        userProfile.userCredentials &&
        userProfile.userCredentials.username
          ? userProfile.userCredentials.username
          : "";
      ...
      $scope.getLegendText = function (description) {
        ...
        return $scope.getDescriptionTextForDescription(
          description,
          $scope.descriptionTypes.full,
          useInStage
        );
      };
      ...
      $scope.verifyEventExpiryDate = function (field) {
        if (!$scope.userAuthority.canEditExpiredStuff) {
          ...
          if (
            $scope.selectedProgram.expiryPeriodType &&
            $scope.selectedProgram.expiryDays
          ) {
            if (
              !DateUtils.verifyExpiryDate(
                date,
                $scope.selectedProgram.expiryPeriodType,
                $scope.selectedProgram.expiryDays,
                true
      ...
      $scope.toggleForm = function (type) {
        if (type === "DEFAULT") {
          $scope.currentStage.timelineDataEntryMode =
            $scope.timelineDataEntryModes.DATAENTRYFORM;
        } else if (type === "COMPPRE") {
          $scope.currentStage.timelineDataEntryMode =
            $scope.timelineDataEntryModes.COMPAREPREVIOUSDATAENTRYFORM;
        } else if (type === "COMPALL") {
          $scope.currentStage.timelineDataEntryMode =
            $scope.timelineDataEntryModes.COMPAREALLDATAENTRYFORM;
        } else if (type === "GRID") {
          $scope.currentStage.timelineDataEntryMode =
            $scope.timelineDataEntryModes.TABLEDATAENTRYFORM;
        ...
        if (!layout.customLayout[$scope.selectedProgramId]) {
          layout.customLayout[$scope.selectedProgramId] =
        ...
        layout.customLayout[
          $scope.selectedProgramId
        ].programStageTimeLineLayout[$scope.currentStage.id] = {
          timelineDataEntryMode: $scope.currentStage.timelineDataEntryMode,
          tableEditMode:
            $scope.tableEditMode || $scope.tableEditMode === 0
              ? $scope.tableEditMode
              : -1,
        };
      ...
      function getCurrentDashboardLayout() {
        ...
        return {
          ...
          program:
            $scope.selectedProgram && $scope.selectedProgram.id
        ...
        angular.forEach($rootScope.ruleeffects[event], function (effect) {
          if (effect.action === "HIDEPROGRAMSTAGE") {
            if (effect.programStage) {
              if (
                $scope.stagesNotShowingInStageTasks[effect.programStage.id] !==
                effect.ineffect
              ) {
                $scope.stagesNotShowingInStageTasks[effect.programStage.id] =
                  effect.ineffect;
              ...
              $log.warn(
                "ProgramRuleAction " +
                  effect.id +
                  " is of type HIDEPROGRAMSTAGE, bot does not have a stage defined"
              );
            }
          }
        });
      ...
      var processRuleEffect = function (event, callerId) {
          ...
          if (effect.action === "HIDEFIELD") {
            if (effect.dataElement) {
              if (
                ...
                if (effect.ineffect && affectedEvent[effect.dataElement.id]) {
                  ...
                  if (effect.content) {
                    ...
                    //TODO: Alerts is going to be replaced with a proper display mecanism.
                    alert(
                      $scope.prStDes[effect.dataElement.id].dataElement
                        .displayFormName +
                        " was blanked out and hidden by your last action"
                    );
                  }

                  //Blank out the value:
                  affectedEvent[effect.dataElement.id] = "";
                  $scope.saveDataValueForEvent(
                    $scope.prStDes[effect.dataElement.id],
                    null,
                    affectedEvent,
                    true
                  );
              ...
              if (!effect.trackedEntityAttribute) {
                $log.warn(
                  "ProgramRuleAction " +
                    effect.id +
                    " is of type HIDEFIELD, bot does not have a field defined"
                );
              }
            }
          } else if (
            ...
            if (effect.ineffect) {
              ...
              if (effect.dataElement && $scope.prStDes[effect.dataElement.id]) {
                ...
                $scope.errorMessages[event].push(
                  $translate.instant(
                    $scope.prStDes[effect.dataElement.id].dataElement
                      .displayName
                  ) +
            ...
            if (effect.ineffect) {
              ...
              if (effect.dataElement && $scope.prStDes[effect.dataElement.id]) {
                if (effect.action === "SHOWWARNING") {
                  ...
                  $scope.warningMessages[event][effect.dataElement.id] =
                    message;
                }
                $scope.warningMessages[event].push(
                  $translate.instant(
                    $scope.prStDes[effect.dataElement.id].dataElement
                      .displayName
                  ) +
            ...
            if (effect.programStageSection) {
              if (effect.ineffect) {
                ...
                !$scope.hiddenSections[event][effect.programStageSection]
              ) {
                $scope.hiddenSections[event][
                  effect.programStageSection
                ] = false;
              }
            } else {
              $log.warn(
                "ProgramRuleAction " +
                  effect.id +
                  " is of type HIDESECTION, bot does not have a section defined"
              );
            }
            ...
            if (
              ...
              if (effect.ineffect && effect.dataElement) {
                ...
                if (
                  $scope.prStDes[effect.dataElement.id].dataElement.optionSet
                ) {
                ...
                processedValue = CommonUtils.formatDataValue(
                  affectedEvent.event,
                  processedValue,
                  prStDe.dataElement,
                  $scope.optionSets,
            ...
            if (effect.dataElement) {
              $scope.mandatoryFields[event][effect.dataElement.id] =
                effect.ineffect;
            }
            ...
            if (effect.ineffect && effect.dataElement && effect.option) {
              if (!$scope.optionVisibility[event][effect.dataElement.id])
                $scope.optionVisibility[event][effect.dataElement.id] = {
                  hidden: {},
                };
              $scope.optionVisibility[event][effect.dataElement.id].hidden[
                effect.option.id
              ] = { id: effect.option.id };
              if (
                dataElementOptionsChanged.indexOf(effect.dataElement.id) === -1
              )
                dataElementOptionsChanged.push(effect.dataElement.id);
            }
              ...
              if (optionGroup) {
                if (
                  ...
                  $scope.optionVisibility[event][
                    effect.dataElement.id
                  ].showOnly = {};
                angular.extend(
                  $scope.optionVisibility[event][effect.dataElement.id]
                    .showOnly,
                  optionGroup.optionsById
                );
                if (
                  dataElementOptionsChanged.indexOf(effect.dataElement.id) ===
                  -1
                )
                  dataElementOptionsChanged.push(effect.dataElement.id);
              } else {
                $log.warn(
                  "OptionGroup " + effect.optionGroup.id + " was not found"
                );
              }
            }
        ...
        clearValueForShowHideOptionActions(
          dataElementOptionsChanged,
          affectedEvent
        );

        updateTabularEntryStages();
        $rootScope.$broadcast("tei-report-widget", {
          events: $scope.allEventsSorted,
        });
      };

      var clearValueForShowHideOptionActions = function (
        dataElements,
        affectedEvent
      ) {
        //Dont process if editing not allowed
        if (
          ["SCHEDULE", "SKIPPED"].indexOf(affectedEvent.status) === -1 &&
          !affectedEvent.editingNotAllowed
        ) {
          dataElements.forEach(function (de) {
            ...
            if (angular.isDefined(value) && value !== "") {
              var optionSet =
                $scope.optionSets[$scope.prStDes[de].dataElement.optionSet.id];
              ...
              if (selectedOption) {
                shouldClear =
                  ($scope.optionVisibility[affectedEvent.event][de].showOnly &&
                    !$scope.optionVisibility[affectedEvent.event][de].showOnly[
                      selectedOption.id
                    ]) ||
                  $scope.optionVisibility[affectedEvent.event][de].hidden[
                    selectedOption.id
                  ];
              }
              ...
              if (shouldClear) {
                var message =
                  $scope.prStDes[de].dataElement.displayFormName +
                  ' was blanked out because the option "' +
                  value +
                  '" got hidden by your last action';
                alert(message);
                affectedEvent[de] = "";
                $scope.saveDataValueForEvent(
                  $scope.prStDes[de],
      ...
      function updateTabularEntryStages() {
        ...
        angular.forEach($scope.programStages, function (programStage) {
          if (
            !$scope.stagesNotShowingInStageTasks[programStage.id] ||
        ...
        $scope.tabularEntryStages = orderByFilter(
          $scope.tabularEntryStages,
          "-sortOrder"
        ).reverse();
      }
      ...
      $scope.mainMenuStageSelected = function () {
        if (
          angular.isDefined($scope.selectedMainMenuStage) &&
          !angular.equals({}, $scope.selectedMainMenuStage)
        ) {
          return true;
        }
        return false;
      ...
      $scope.buildMainMenuStages = function () {
        ...
        angular.forEach($scope.programStages, function (stage) {
          if (
            (angular.isUndefined($scope.neverShowItems) ||
              angular.isUndefined($scope.neverShowItems[stage.id]) ||
              $scope.neverShowItems[stage.id] === false) &&
            (angular.isUndefined($scope.headerCombineStages) ||
              angular.isUndefined($scope.headerCombineStages[stage.id]))
          ) {
        ...
        angular.forEach($scope.programStages, function (stage) {
          if (
            (angular.isUndefined($scope.bottomLineItems) ||
              angular.isUndefined($scope.bottomLineItems[stage.id]) ||
              $scope.bottomLineItems[stage.id] === false) &&
            (angular.isUndefined($scope.neverShowItems) ||
              angular.isUndefined($scope.neverShowItems[stage.id]) ||
              $scope.neverShowItems[stage.id] === false) &&
            (angular.isUndefined($scope.headerCombineStages) ||
      ...
      $scope.headerStagesWithoutCurrent = function () {
        if ($scope.headerStages.length > 0) {
          if (angular.isDefined($scope.selectedMainMenuStage)) {
            ...
            if (
              angular.isDefined($scope.headerCombineStages) &&
              $scope.headerCombineStages[$scope.selectedMainMenuStage.id]
            ) {
              currentStage =
                $scope.stagesById[
                  $scope.headerCombineStages[$scope.selectedMainMenuStage.id]
      ...
      $scope.headerCurrentStageName = function () {
        ...
        if (
          $scope.selectedMainMenuStage &&
          angular.isDefined($scope.selectedMainMenuStage.displayName)
        ) {
          ...
          if (
            ...
            if (
              angular.isDefined(stageWithName) &&
              angular.isObject(stageWithName)
            ) {
      ...
      function topLineEventsIsFiltered() {
        if (
          (angular.isDefined($scope.neverShowItems) &&
            !angular.equals({}, $scope.neverShowItems)) ||
          (angular.isDefined($scope.bottomLineItems) &&
            !angular.equals({}, $scope.bottomLineItems)) ||
          (angular.isDefined($scope.topLineStageFilter) &&
            !angular.equals({}, $scope.topLineStageFilter))
        ) {
          return true;
        }
        return false;
      ...
      $scope.getTopLineEventsPage = function () {
        if ($scope.allEventsSorted && $scope.allEventsSorted.length > 0) {
          ...
          return topLineEvents.slice(
            $scope.eventPagingStart,
            $scope.eventPagingEnd
          );
        }
        return [];
      };
      ...
      $scope.isCompulsory = function (dataElement) {
        return $scope.currentStage.programStageDataElementsCollection[
          dataElement.id
        ].compulsory;
      };
      ...
      $scope.isHidden = function (id, event) {
        ...
        var EventToCheck = angular.isDefined(event)
          ? event
          : $scope.currentEvent;
      ...
      $scope.executeRules = function () {
        ...
        for (var ps = 0; ps < $scope.programStages.length; ps++) {
          for (
            var e = 0;
            e < $scope.eventsByStage[$scope.programStages[ps].id].length;
            e++
          ) {
            allSorted.push(
              $scope.eventsByStage[$scope.programStages[ps].id][e]
            );
          }
        }
        ...
        if (
          $scope.currentStage &&
          $scope.currentStage.displayEventsInTable &&
          angular.isUndefined($scope.currentStage.rulesExecuted)
        ) {
      ...
      $scope.$on("dashboardWidgets", function () {
        ...
        $scope.showSelf = true;
        if (
          angular.isUndefined($scope.selectedEnrollment) ||
          $scope.selectedEnrollment === null ||
          ($scope.dashBoardWidgetFirstRun &&
        ...
        if (
          ...
          if ($scope.selectedOrgUnit.reportDateRange) {
            if ($scope.selectedOrgUnit.reportDateRange.minDate) {
              $scope.model.minDate =
                $scope.selectedOrgUnit.reportDateRange.minDate;
              //minDate is in Georgian format, but maxDate is not. This Service converts the date.
              $scope.model.minDate = DateUtils.formatFromApiToUserCalendar(
                $scope.model.minDate
              );
              $scope.model.minDate = DateUtils.formatFromApiToUser(
                $scope.model.minDate
              );
            }
            if ($scope.selectedOrgUnit.reportDateRange.maxDate) {
              $scope.model.maxDate =
                $scope.selectedOrgUnit.reportDateRange.maxDate;
            }
          }

          $scope.programStages = $scope.tabularEntryStages =
          ...
          angular.forEach(
            ...
            function (stage) {
              ...
              stage.executionDateLabel = stage.executionDateLabel
                ? stage.executionDateLabel
                : $translate.instant("report_date");
              stage.dueDateLabel = stage.dueDateLabel
                ? stage.dueDateLabel
          ...
          $scope.programStages = orderByFilter(
            $scope.programStages,
            "-sortOrder"
          ).reverse();
          ...
          if (
            $scope.selectedProgram.categoryCombo &&
          ...
          TrackerRulesFactory.getRules($scope.selectedProgram.id).then(
            function (rules) {
      ...
      $scope.deleteScheduleAndOverdueEvents = function () {
        ...
        for (var i = 0; i < $scope.programStages.length; i++) {
          for (
            var e = 0;
            e < $scope.eventsByStage[$scope.programStages[i].id].length;
            e++
          ) {
            if (
              $scope.eventsByStage[$scope.programStages[i].id][e].status ===
                "SCHEDULE" ||
              $scope.eventsByStage[$scope.programStages[i].id][e].status ===
                "OVERDUE"
            ) {
              promises.push(
                DHIS2EventFactory.delete(
                  $scope.eventsByStage[$scope.programStages[i].id][e]
                )
              );
            }
          }
      ...
      $scope.stageCanBeShownAsTable = function (stage) {
        if (
          stage.programStageDataElements &&
          stage.programStageDataElements.length <=
            $scope.tableMaxNumberOfDataElements &&
          stage.repeatable
        ) {
          return true;
        }
      ...
      $scope.stageNeedsEventOfType = function (
        stage,
        type,
        completeRequired,
        ...
        if (
          type === $scope.eventCreationActions.schedule ||
          type === $scope.eventCreationActions.referral
        ) {
          if (stage.hideDueDate === true) {
            if (angular.isDefined(errorResponseContainer)) {
              errorResponseContainer.errorCode =
                $scope.stageNeedsEventErrors.scheduleDisabled;
            }
            return false;
          }
        }

        return $scope.stageNeedsEvent(
          stage,
          completeRequired,
          errorResponseContainer
        );
      ...
      $scope.stageNeedsEvent = function (
        stage,
        completeRequired,
        errorResponseContainer
      ) {
        ...
        var calculatedCompleteRequired =
          (angular.isDefined(completeRequired) && completeRequired) ||
          (angular.isDefined(stage.onlyOneIncompleteEvent) &&
            stage.onlyOneIncompleteEvent);
        ...
        if (
          ...
          if (!stage) {
            if (
              !$scope.allEventsSorted ||
              $scope.allEventsSorted.length === 0
            ) {
              return true;
            }
            for (var key in $scope.eventsByStage) {
              ...
              if (stage && stage.repeatable) {
                for (
                  var j = 0;
                  j < $scope.eventsByStage[stage.id].length;
                  j++
                ) {
                  if (
                    !$scope.eventsByStage[stage.id][j].eventDate &&
                    $scope.eventsByStage[stage.id][j].status !== "SKIPPED"
                  ) {
                    return true;
                  }
                }
          ...
          if (stage.repeatable) {
            for (var j = 0; j < $scope.eventsByStage[stage.id].length; j++) {
              if (
                angular.isDefined(calculatedCompleteRequired) &&
                calculatedCompleteRequired === true
              ) {
                var foundEvent = $scope.eventsByStage[stage.id][j];
                if (
                  foundEvent.status !== $scope.EVENTSTATUSCOMPLETELABEL &&
                  foundEvent.status !== $scope.EVENTSTATUSSKIPPEDLABEL
                ) {
                  if (angular.isDefined(errorResponseContainer)) {
                    errorResponseContainer.errorCode =
                      $scope.stageNeedsEventErrors.complete;
                  }
                  return false;
                }
              } else if (
                ...
                if (angular.isDefined(errorResponseContainer)) {
                  errorResponseContainer.errorCode =
                    $scope.stageNeedsEventErrors.scheduledFound;
                }
                return false;
              }
            }
            ...
            if (angular.isDefined(errorResponseContainer)) {
              errorResponseContainer.errorCode =
                $scope.stageNeedsEventErrors.notRepeatable;
            }
            return false;
          }
        }
        ...
        if (angular.isDefined(errorResponseContainer)) {
          errorResponseContainer.errorCode =
            $scope.stageNeedsEventErrors.enrollment;
        }
        return false;
      };
      ...
      $scope.getTopLineColumnStyle = function (colNr) {
        if (
          $scope.useMainMenu &&
          ($scope.topLineEvents.length === 0 ||
            $scope.hideTopLineEventsForFormTypes[$scope.displayCustomForm])
        ) {
          if (colNr === 1) {
            return "col-xs-12";
          } else {
          ...
          if (colNr === 1) {
            return $scope.selectedEnrollment.status !== "ACTIVE"
              ? "col-xs-12"
              : "col-xs-6 col-sm-9";
          } else {
      ...
      $scope.addNewEvent = function (newEvent, setProgramStage) {
        ...
        newEvent = EventUtils.processEvent(
          newEvent,
          $scope.stagesById[newEvent.programStage],
          $scope.optionSets,
          $scope.prStDes
        );
        if (setProgramStage)
          $scope.currentStage = $scope.stagesById[newEvent.programStage];
      ...
      $scope.showCreateEventIfStageNeedsEvent = function (
        stage,
        eventCreationAction,
        requireStageEventsToBeCompleted,
        showModalOnNoEventsNeeded
      ) {
        showModalOnNoEventsNeeded =
          angular.isDefined(showModalOnNoEventsNeeded) &&
          showModalOnNoEventsNeeded === true
            ? true
            : false;
        var errorResponseContainer = {};

        if (
          $scope.stageNeedsEventOfType(
            stage,
            eventCreationAction,
            requireStageEventsToBeCompleted,
            errorResponseContainer
            ...
            if (angular.isDefined(errorResponseContainer.errorCode)) {
              switch (errorResponseContainer.errorCode) {
                ...
                case $scope.stageNeedsEventErrors.notRepeatable:
                  errorMessage = $translate.instant(
                    "programstage_multiple_events_disabled"
                  );
                  break;
                default:
                  break;
      ...
      $scope.showCreateEvent = function (
        ...
        if (stage) {
          if (!stage.access.data.write) {
            ...
            var bodyText = $translate.instant(
              "you_need_write_to_program_stage_to_be_able_to_create_event"
            );
            NotificationService.showNotifcationDialog(headerText, bodyText);
            return;
          }
          ...
          if (!$scope.allEventsSorted || $scope.allEventsSorted.length === 0) {
            ...
            angular.forEach($scope.allEventsSorted, function (event) {
              if (
                angular.isUndefined(
                  $scope.stagesNotShowingInStageTasks[event.programStage]
                )
              ) {
        ...
        var autoCreate =
          stage && stage.displayEventsInTable
            ? stage.displayEventsInTable
            : false;
        EventCreationService.showModal(
          $scope.eventsByStage,
          stage,
          availableStages,
          writableStages,
      ...
      $scope.setCurrentStage = function (stage, openDataEntry) {
        ...
        $scope.currentStage = stage;
        $scope.currentEvent = null;
        $scope.eventGridColumns = EventUtils.getGridColumns(
          $scope.currentStage,
          $scope.prStDes
        );
        ...
        if ($scope.currentEvent) {
          $scope.dashBoardLayout
            ? $scope.getDataEntryForm()
            : dashBoardLayoutPromise.then(getDataEntryForm);
        }
      };
          ...
          if (
            ...
            dashBoardLayoutPromise.then(function () {
              TCOrgUnitService.get(event.orgUnit).then(function (orgUnit) {
                ...
                for (
                  var i = 0;
                  i < $scope.eventsByStage[event.programStage].length &&
                  index === -1;
                  i++
                ) {
                  if (
                    $scope.eventsByStage[event.programStage][i].event ===
                    event.event
                  ) {
                    index = i;
                  }
                ...
                if (index !== -1) {
                  $scope.currentEvent =
                    $scope.eventsByStage[event.programStage][index];
                }
                ...
                if ($scope.currentEvent.notes) {
                  angular.forEach($scope.currentEvent.notes, function (note) {
                    UsersService.getByQuery(note.storedBy).then(function (
                      users
                    ) {
                      if (users.length === 1) {
                        note.storedBy =
                    ...
                    note.storedDate = DateUtils.formatToHrsMins(
                      note.storedDate
                    );
                  });

                  if ($scope.currentEvent.notes.length > 0) {
                    $scope.currentEvent.notes = orderByFilter(
      ...
      $scope.tableRowStatusButtonsEnabled = function (event) {
        return (
          event.orgUnit === $scope.selectedOrgUnit.id &&
          $scope.selectedEntity.inactive === false &&
          $scope.selectedEnrollment.status === "ACTIVE"
        );
      };
      ...
      $scope.openEventEditFormModal = function (event) {
        var stage = $scope.stagesById[event.programStage];
        if (stage && stage.id) {
          event.editingNotAllowed = EventUtils.getEditingStatus(
            event,
            stage,
            $scope.selectedOrgUnit,
            $scope.selectedTei,
        ...
        $scope.eventEditFormModalInstance.result.then(
          function (status) {
      ...
      $scope.buildOtherValuesLists = function () {
        ...
        if (
          ...
          angular.forEach(
            $scope.currentStage.programStageDataElements,
            function (programStageDataElement) {
              angular.forEach($scope.programStages, function (stage) {
                for (
                  var i = 0;
                  i < $scope.eventsByStage[stage.id].length;
                  i++
                ) {
                  //We are not interested in the values from the current stage:
                  if (
                    $scope.eventsByStage[stage.id][i] !== $scope.currentEvent
                  ) {
                    ...
                    if ($scope.eventsByStage[stage.id][i][currentId]) {
                      ...
                      var alternateValue =
                        $scope.eventsByStage[stage.id][i][currentId];
                      alternateValue = CommonUtils.displayBooleanAsYesNo(
                        alternateValue,
                      ...
                      alternateValue =
                        $scope.eventsByStage[stage.id][i].eventDate +
                        ": " +
                        alternateValue;
      ...
      $scope.getDataEntryForm = function () {
        ...
        $scope.currentFileNames = $scope.fileNames
          ...
          : [];
        $scope.currentStage =
          $scope.stagesById[$scope.currentEvent.programStage];
        $scope.currentStageEvents =
        ...
        if (
          ...
          $scope.dashBoardLayout.customLayout[$scope.selectedProgramId]
            .programStageTimeLineLayout[$scope.currentStage.id] &&
          !$scope.lockedList[$scope.selectedProgramId]
        ) {
          $scope.currentStage.timelineDataEntryMode =
            $scope.dashBoardLayout.customLayout[
              $scope.selectedProgramId
            ...
            ].timelineDataEntryMode;
          $scope.currentStage.tableEditMode =
            $scope.dashBoardLayout.customLayout[
              $scope.selectedProgramId
            ].programStageTimeLineLayout[$scope.currentStage.id].tableEditMode;
        } else if (
          $scope.dashBoardLayout &&
          $scope.dashBoardLayout.defaultLayout[$scope.selectedProgramId] &&
          $scope.dashBoardLayout.defaultLayout[$scope.selectedProgramId]
        ...
        angular.forEach(
          $scope.currentStage.programStageSections,
          function (section) {
            section.open = true;
          }
        );
        ...
        $scope.customDataEntryForm = CustomFormService.getForProgramStage(
          $scope.currentStage,
          $scope.prStDes
        );
          ...
          switch ($scope.currentStage.timelineDataEntryMode) {
            ...
            case $scope.timelineDataEntryModes.TABLEDATAENTRYFORM:
              if ($scope.reSortStageEvents === true) {
                ...
                if (
                  $scope.eventsByStage.hasOwnProperty($scope.currentStage.id)
                ) {
                  $scope.currentStageEvents =
        ...
        $scope.currentEvent.editingNotAllowed = EventUtils.getEditingStatus(
          $scope.currentEvent,
          $scope.currentStage,
          $scope.selectedOrgUnit,
          $scope.selectedTei,
        ...
        $scope.currentStageEventsOriginal = angular.copy(
          $scope.currentStageEvents
        );
        ...
        $rootScope.$broadcast("dataEntryEventChanged", {
          event: $scope.currentEvent.event,
        });
        $scope.executeRules();
      };
      ...
      $scope.saveDataValueForRadio = function (prStDe, event, value) {
        ...
        var saveReturn = $scope.saveDataValueForEvent(
          prStDe,
          null,
          event,
          false
      ...
      $scope.saveDataValueForEvent = function (
        prStDe,
        field,
        eventToSave,
        backgroundUpdate
        ...
        for (var i = 0; i < $scope.currentStageEventsOriginal.length; i++) {
          if (
            $scope.currentStageEventsOriginal[i].event === eventToSave.event
          ) {
            oldValue =
              $scope.currentStageEventsOriginal[i][prStDe.dataElement.id];
            break;
          }
        }
        ...
        if (
          field &&
          field.$invalid &&
          prStDe.dataElement.valueType !== "ORGANISATION_UNIT"
        ) {
        ...
        if (oldValue !== value) {
          value = CommonUtils.formatDataValue(
            eventToSave.event,
            value,
            prStDe.dataElement,
            $scope.optionSets,
            ...
            dataValues: [
              {
                ...
                value: value,
                providedElsewhere: eventToSave.providedElsewhere[
                  prStDe.dataElement.id
                ]
                  ? true
                  : false,
          ...
          return DHIS2EventFactory.updateForSingleValue(ev).then(function (
            response
          ) {
            if (!response) {
              if (!backgroundUpdate) {
                ...
                $log.warn(
                  "Could not perform background update of " +
                    prStDe.dataElement.id +
                    " with value " +
                    value
                );
            ...
            dataValues: [
              {
                dataElement: prStDe.dataElement.id,
                value: value,
                providedElsewhere: $scope.currentEvent.providedElsewhere[
                  prStDe.dataElement.id
                ]
                  ? true
                  : false,
      ...
      $scope.saveEventDateForEvent = function (eventToSave, reOrder) {
        ...
        var e = {
          ...
          status:
            eventToSave.status === "SCHEDULE" ? "ACTIVE" : eventToSave.status,
          program: eventToSave.program,
          programStage: eventToSave.programStage,
          orgUnit:
            eventToSave.dataValues && eventToSave.dataValues.length > 0
              ? eventToSave.orgUnit
              : $scope.selectedOrgUnit.id,
          orgUnitName:
            eventToSave.dataValues && eventToSave.dataValues.length > 0
              ? eventToSave.orgUnitName
              : $scope.selectedOrgUnit.displayName,
          orgUnitPath:
            eventToSave.dataValues && eventToSave.dataValues.length > 0
              ? eventToSave.orgUnitPath
              : $scope.selectedOrgUnit.path,
          eventDate: DateUtils.formatFromUserToApi(eventToSave.eventDate),
          trackedEntityInstance: eventToSave.trackedEntityInstance,
        };

        DHIS2EventFactory.updateForEventDate(e).then(function (data) {
          eventToSave.sortingDate = DateUtils.formatFromUserToApi(
      ...
      $scope.saveDueDate = function () {
        ...
        DHIS2EventFactory.update(e).then(function (data) {
          ...
          if (
            e.eventDate &&
            !$scope.currentEvent.eventDate &&
            $scope.currentStage.periodType
          ) {
            $scope.currentEvent.eventDate = $scope.currentEvent.dueDate;
          }

          $scope.currentEvent.sortingDate = $scope.currentEvent.dueDate;
          $scope.currentEvent.statusColor = EventUtils.getEventStatusColor(
            $scope.currentEvent
          );
      ...
      $scope.notesModal = function () {
        ...
        var dialogOptions = {
          ...
          textAreaButtonShow:
            $scope.currentEvent.status === $scope.EVENTSTATUSSKIPPEDLABEL
              ? false
              : true,
          headerText: "Notes",
          bodyTextAreas: [
            ...
            $scope.addNote = function () {
              ...
              var e = {
                ...
                trackedEntityInstance:
                  $scope.currentEvent.trackedEntityInstance,
                notes: [newNote],
              };
              DHIS2EventFactory.updateForNote(e).then(function (data) {
                if (
                  angular.isUndefined($scope.modalOptions.bodyList) ||
                  $scope.modalOptions.bodyList.length === 0
                ) {
                  $scope.modalOptions.bodyList = [
                    { value1: date, value2: newNote.value },
                  ];
        ...
        NotificationService.showNotifcationWithOptions(
          dialogDefaults,
          dialogOptions
        ).then(function (e) {
      ...
      $scope.makeDhis2EventToUpdate = function () {
        var dhis2Event = EventUtils.reconstruct(
          $scope.currentEvent,
          $scope.currentStage,
          $scope.optionSets
        );
      ...
      $scope.completeIncompleteEvent = function (
        ...
        if ($scope.currentEvent.status !== "COMPLETED") {
          ...
          if ($scope.outerDataEntryForm.$invalid) {
            NotificationService.showNotifcationDialog(
              ...
              $translate.instant("form_invalid")
            );
            return;
          }
        }
        ...
        if ($scope.currentEvent.status === "COMPLETED") {
          ...
          if (
            angular.isUndefined(inTableView) ||
            inTableView === false ||
            inTableView === null
          ) {
          ...
          if (
            angular.isDefined(
              $scope.warningMessages[$scope.currentEvent.event]
            ) &&
            $scope.warningMessages[$scope.currentEvent.event].length > 0
          ) {
            warningSection = {
          ...
          if (
            angular.isDefined(
              $scope.errorMessages[$scope.currentEvent.event]
            ) &&
            $scope.errorMessages[$scope.currentEvent.event].length > 0
          ) {
            ...
            modalDefaults.templateUrl =
              "components/dataentry/modal-complete-event.html";
        ...
        ModalService.showModal(modalDefaults, modalOptions).then(function (
          ...
          if (
            modalResult === modalCompleteIncompleteActions.completeEnrollment
          ) {
            modalOptions = {
              closeButtonText: "cancel",
      ...
      $scope.executeCompleteIncompleteEvent = function (
        dhis2Event,
        modalResult
      ) {
        var modalOptions = {};
        ...
        return DHIS2EventFactory.update(dhis2Event).then(function (data) {
          if (modalResult === modalCompleteIncompleteActions.completeAndExit) {
            ...
            for (var i = 0; i < $scope.allEventsSorted.length; i++) {
              if (
                $scope.allEventsSorted[i].event === $scope.currentEvent.event
              ) {
            ...
            if ($scope.currentEvent.status === "COMPLETED") {
              if (
                modalResult ===
                modalCompleteIncompleteActions.completeEnrollment
              ) {
                completeEnrollment();
              } else {
                if ($scope.currentStage.allowGenerateNextVisit) {
                  $scope.showCreateEvent(
                    null,
                    $scope.eventCreationActions.schedule
                  );
                }
              }
            }
      ...
      var setStatusColor = function () {
        ...
        for (
          var i = 0;
          i < $scope.eventsByStage[$scope.currentEvent.programStage].length &&
          continueLoop;
          i++
        ) {
          if (
            $scope.eventsByStage[$scope.currentEvent.programStage][i].event ===
            $scope.currentEvent.event
          ) {
            $scope.eventsByStage[$scope.currentEvent.programStage][
              i
      ...
      $scope.dataElementEditable = function (prStDe) {
        if ($scope.eventEditable()) {
          if (
            $scope.assignedFields[$scope.currentEvent.event][
              prStDe.dataElement.id
            ]
          )
            return false;
      ...
      var isInSearchOrgUnits = function (orgUnitPath, searchOrgUnits) {
        ...
        return TCOrgUnitService.isPathInOrgUnitList(
          orgUnitPath,
          searchOrgUnits
        );
      };
      ...
      $scope.reportDateEditable = function () {
        ...
        if (!$scope.currentStage || !$scope.currentStage.access.data.write)
          return false;
        ...
        if (
          ...
          !(
            $scope.currentEvent.status === "SCHEDULE" &&
            isInSearchOrgUnits(
              $scope.currentEvent.orgUnitPath,
              userSearchOrgUnits
            )
          )
        )
        ...
        if (
          $scope.currentStage &&
          $scope.currentStage.blockEntryForm &&
          $scope.currentEvent.status === "COMPLETED"
        )
          return false;
        ...
        if (
          $scope.currentEvent.expired &&
          !$scope.userAuthority.canEditExpiredStuff
        )
          return false;
      ...
      $scope.eventEditable = function (isButton) {
        if (
          !$scope.currentStage ||
          !$scope.currentStage.access ||
          !$scope.currentStage.access.data.write
        )
          return false;
        if (
          $scope.selectedOrgUnit.closedStatus ||
          $scope.selectedEnrollment.status !== "ACTIVE"
        )
          return false;
        if (isButton) {
          if (
            !$scope.currentEvent ||
            ($scope.currentEvent.editingNotAllowed &&
              !$scope.userAuthority.canUncompleteEvent) ||
            ($scope.currentEvent.expired &&
              !$scope.userAuthority.canEditExpiredStuff)
          )
            return false;
      ...
      $scope.deleteEvent = function () {
        if (!$scope.canDeleteEvent()) {
          var bodyText =
            $translate.instant(
              "you_do_not_have_the_necessary_authorities_to_delete"
            ) +
            " " +
            $translate.instant("this") +
            " " +
            $translate.instant("event").toLowerCase();
          var headerText = $translate.instant("delete_failed");
          return NotificationService.showNotifcationDialog(
            headerText,
            bodyText
          );
        }
      ...
      $scope.executeDeleteEvent = function () {
        return DHIS2EventFactory.delete($scope.currentEvent).then(
          function (data) {
            ...
            if (
              $scope.allEventsSorted[$scope.allEventsSorted.length - 1]
                .event === $scope.currentEvent.event
            ) {
            ...
            if ($scope.isTabular) {
              if ($scope.currentStageEvents.length > 0) {
                if (index === 0) {
                  $scope.showDataEntry(
                    $scope.currentStageEvents[0],
                    false,
                    false
                  );
                } else {
                  $scope.showDataEntry(
                    $scope.currentStageEvents[index - 1],
                    false,
                    false
                  );
                }
          ...
          function (error) {
            ...
            NotificationService.showNotifcationDialog(
              $translate.instant("error"),
              $translate.instant("delete_error_audit")
            );
      ...
      $scope.getEventStyle = function (ev, skipCurrentEventStyle) {
        ...
        $scope.eventStyleLabels[ev.event] =
          $scope.getDescriptionTextForEventStyle(
            style,
            $scope.descriptionTypes.label,
            false
          );

        if (
          $scope.currentEvent &&
          $scope.currentEvent.event === ev.event &&
          (angular.isUndefined(skipCurrentEventStyle) ||
            skipCurrentEventStyle === false)
        ) {
      ...
      var sortStageEvents = function (stage) {
        ...
        if ($scope.eventsByStage.hasOwnProperty(key)) {
          var sortedEvents = $filter("orderBy")(
            $scope.eventsByStage[key],
            function (event) {
              if (
                angular.isDefined(stage.displayEventsInTable) &&
          ...
          if (
            angular.isDefined($scope.currentStage) &&
            $scope.currentStage !== null &&
            $scope.currentStage.id === stage.id
          ) {
      ...
      var sortEventsByStage = function (operation, newEvent) {
        ...
        for (var key in $scope.eventsByStage) {
          ...
          if (
            $scope.eventsByStage.hasOwnProperty(key) &&
            stage &&
            $scope.selectedEnrollment &&
            ...
            $scope.eventFilteringRequired = $scope.eventFilteringRequired
              ? $scope.eventFilteringRequired
              : periods.length > 1;
          }
        }
        ...
        if (operation) {
          ...
          if (operation === "UPDATE") {
            ...
            for (
              var i = 0;
              i < $scope.allEventsSorted.length && index === -1;
              i++
            ) {
              if (
                $scope.allEventsSorted[i].event === $scope.currentEvent.event
      ...
      $scope.showLastEventInStage = function (stageId) {
        var ev =
          $scope.eventsByStage[stageId][
            $scope.eventsByStage[stageId].length - 1
          ];
      ...
      $scope.getEventPage = function (direction) {
        ...
        $scope.showDataEntry(
          $scope.topLineEvents[$scope.eventPagingStart],
          false,
          true
        );
      };
      ...
      $scope.getEventPageForEvent = function (event) {
        if (
          angular.isDefined(event) &&
          angular.isObject(event) &&
          angular.isDefined($scope.topLineEvents)
        ) {
          var index = -1;
      ...
      $scope.showCompleteErrorMessageInModal = function (
        message,
        fieldName,
        isWarning
      ) {
        ...
        var headerPrefix =
          angular.isDefined(isWarning) && isWarning === true
            ? "Warning in "
            : "Error in ";
      ...
      $scope.setOtherStageEvents = function () {
        ...
        if (
          angular.isUndefined($scope.currentStageEvents) ||
          $scope.currentStageEvents.length <= 1
        ) {
          return;
        } else {
          angular.forEach($scope.currentStageEvents, function (event) {
            if (
              event.status !== $scope.EVENTSTATUSSKIPPEDLABEL ||
              event.event === $scope.currentEvent.event
            ) {
          ...
          if ($scope.compareMode === $scope.compareDataEntryFormModes.ALL) {
            var subsequentEventsCount =
              $scope.maxCompareItemsInCompareView -
              $scope.otherStageEventIndexes.length;
            if (subsequentEventsCount > 0) {
              for (var k = 0; k < subsequentEventsCount; k++) {
                ...
                var relative = k + 1;
                $scope.otherStageEventIndexes.push({
                  relative: relative,
                  position: position,
                });
              }
          ...
          angular.forEach(
            $scope.otherStageEventIndexes,
            function (indexContainer, i, j) {
              $scope.otherStageEvents.push(
                $scope.stageEventsExcludedSkipped[indexContainer.position]
              );
            }
          );
        }
        ...
        if (type === $scope.buttonType.back) {
          if ($scope.otherStageEventIndexes.length > 0) {
            ...
            if (indexContainer.position > 0 && indexContainer.relative < 0) {
              ...
              $scope.compareMode === $scope.compareDataEntryFormModes.ALL &&
              indexContainer.position > 0 &&
              indexContainer.relative > 0
            ) {
          ...
          if ($scope.otherStageEventIndexes.length > 0) {
            var lastEvent =
              $scope.otherStageEventIndexes[
                $scope.otherStageEventIndexes.length - 1
              ];
            if (lastEvent.relative < -1) {
              ...
              var hasNextEvents =
                totalEvents - 1 - $scope.indexOfCurrentEvent > 0 &&
                lastEvent.position < totalEvents - 1;
      ...
      $scope.currentStageEventNumber = function () {
        for (var i = 0; i < $scope.stageEventsExcludedSkipped.length; i++) {
          if (
            $scope.currentEvent.event ===
            $scope.stageEventsExcludedSkipped[i].event
          ) {
            return i + 1;
          }
        }
      ...
      $scope.getStageStyle = function (stage) {
        ...
        if (eventToFetch >= 0) {
          var stageStyle = $scope.getEventStyle(
            $scope.eventsByStage[stage.id][eventToFetch]
          );
      ...
      $scope.getDescriptionTextForEventStyle = function (
        style,
        descriptionType,
        useInStage
      ) {
        if (angular.isDefined(style) && style !== "") {
          var eventStyles = $filter("filter")(
            $scope.eventStyles,
            { color: style },
            true
          );
      ...
      $scope.getDescriptionTextForDescription = function (
        description,
        descriptionType,
        useInStage
      ) {
      ...
      $scope.getEventForStage = function (stage) {
        ...
        if (
          angular.isDefined($scope.eventsByStage[stage.id]) &&
          $scope.eventsByStage[stage.id].length > 0
        ) {
          ...
          for (var i = 0; i < stageEvents.length; i++) {
            ...
            if (
              itiratedEvent.status !== $scope.EVENTSTATUSSKIPPEDLABEL &&
              itiratedEvent.status !== $scope.EVENTSTATUSCOMPLETELABEL
            ) {
              firstOpen = i;
              break;
            } else if (
      ...
      $scope.getEventForStages = function (stagesInputArray) {
        ...
        angular.forEach(stages, function (stage) {
          eventsForStages = eventsForStages.concat(
            $scope.eventsByStage[stage.id]
          );
        });
      ...
      $scope.openStageEvent = function (stage, noEventFoundFunc) {
        ...
        if (latest >= 0) {
          $scope.getEventFromStageSelection(
            $scope.eventsByStage[stage.id][latest]
          );
        } else {
          noEventFoundFunc();
        }
      ...
      $scope.getEventFromStageSelection = function (event) {
        ...
        if (
          angular.isDefined($scope.currentStage) &&
          $scope.currentStage !== null &&
          angular.isDefined($scope.currentStage.displayEventsInTable) &&
          ...
          if (angular.isDefined($scope.headerCombineStages)) {
            for (var key in $scope.headerCombineStages) {
              if (stage.id === key) {
                if (
                  angular.isDefined(
                    $scope.eventsByStage[$scope.headerCombineStages[key]]
                  )
                ) {
                  cnt +=
                    $scope.eventsByStage[$scope.headerCombineStages[key]]
          ...
          switch ($scope.stageErrorInEventLayout[stage.id]) {
            case $scope.eventCreationActions.add:
              return $translate.instant(
                "please_complete_all_results_before_add"
              );
              break;
            case $scope.eventCreationActions.schedule:
              return $translate.instant(
                "please_complete_all_results_before_schedule"
              );
              break;
            case $scope.eventCreationActions.referral:
              return $translate.instant(
                "please_complete_all_results_before_referral"
              );
              break;
          }
        }
      ...
      $scope.downloadFile = function (eventUid, dataElementUid, e) {
        ...
        if (!eventUid || !dataElementUid) {
          NotificationService.showNotifcationDialog(
            $translate.instant("error"),
            $translate.instant("missing_file_identifier")
          );
          return;
        }

        $window.open(
          "../api/events/files?eventUid=" +
            eventUid +
            "&dataElementUid=" +
            dataElementUid,
          "_blank",
      ...
      $scope.updateFileNames = function () {
        Object.keys($scope.currentFileNames).forEach(function (dataElement) {
          if ($scope.currentFileNames[dataElement]) {
            ...
            $scope.fileNames[$scope.currentEvent.event][dataElement] =
              $scope.currentFileNames[dataElement];
          }
        });
      };

      $scope.editAttributeCategoryOptions = function () {
        $scope.showAttributeCategoryOptions =
          !$scope.showAttributeCategoryOptions;

        if (
          $scope.showAttributeCategoryOptions &&
          $scope.currentEvent &&
          $scope.currentEvent.attributeCategoryOptions
        ) {
          var selectedOptions =
            $scope.currentEvent.attributeCategoryOptions.split(";");
          var reverse = false;
          for (var i = 0; i < $scope.selectedCategories.length; i++) {
            for (
              var j = 0;
              j < $scope.selectedCategories[i].categoryOptions.length;
              j++
            ) {
              if (
                $scope.selectedCategories[i].categoryOptions[j].id ===
                selectedOptions[i]
              ) {
                $scope.selectedCategories[i].selectedOption =
                  $scope.selectedCategories[i].categoryOptions[j];
                break;
              } else {
                reverse = true;
              }
      ...
      $scope.saveAttributeCategoryOptions = function () {
        ...
        for (var i = 0; i < $scope.selectedCategories.length; i++) {
          if (
            $scope.selectedCategories[i].selectedOption.id !==
            $scope.currentEvent.attributeCategoryOptions.split(";")[i]
          ) {
        ...
        if (
          optionsReady &&
          acos !== $scope.currentEvent.attributeCategoryOptions
        ) {
          var ev = EventUtils.processEvent(
            $scope.currentEvent,
            $scope.currentStage,
            $scope.optionSets,
            $scope.prStDes
          ...
          if (
            ...
            ev.attributeCategoryOptions &&
            ev.attributeOptionCombo
          ) {
            var dhis2Event = {
              event: ev.event,
              orgUnit: ev.orgUnit,
          ...
          DHIS2EventFactory.update(dhis2Event).then(function (data) {
            $scope.currentEvent.attributeCategoryOptions =
              dhis2Event.attributeCategoryOptions;
      ...
      $scope.calcTableWidth = function () {
        var width = angular.element(
          document.getElementById("tabelContainer")
        )[0].clientWidth;
        return width;
      };
  ...
  .controller("EventOptionsInTableController", function ($scope, $translate) {
    ...
    function createOptionsArray() {
      ...
      for (var key in $scope.eventTableOptions) {
        $scope.eventTableOptionsArr[$scope.eventTableOptions[key].sort] =
          $scope.eventTableOptions[key];
      }
    }
  });
